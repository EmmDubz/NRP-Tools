...
- **View → Strict Ocean Mask:** when enabled (default), **Luminance** and **ΔE heat** de-emphasize pixels that match **`ocean_colors`** on the political map so land reads more clearly. **Mask — commodity** and **Hits only** always show every classified hit (including offshore oil / natural_gas on water); that matches **`analyze_resources.py`** totals (`pixels_offshore`). Turn Strict Ocean Mask off if you want sea at full strength in Luminance/ΔE. **Diagnostic — void + oil/gas tie hatch** tints **magenta** where **no** commodity is within its ΔE gate (anchors/thresholds), and **red diagonal hatch** where oil and natural_gas are both in-range and ΔE is within `oil_gas_ambiguity_delta` (same idea as the propagation guard).
